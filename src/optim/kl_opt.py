    ...
    def step(self, closure = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                state = self.state[p]
                use_preconditioner = grad.dim() in (2, 3) and max(grad.shape) <= group["max_precond_dim"]
                ...
                if "exp_avg" not in state:
                    ...
                    if self.using_klsoap or not use_preconditioner:
                ...
                if not use_preconditioner:
                    ...
                    exp_avg.lerp_(grad, 1.0-group["betas"][0])
                    exp_avg_sq.lerp_(grad.square(), 1.0-group["betas"][1])
                    norm_grad = exp_avg / exp_avg_sq.sqrt().add_(group['eps'])

                    if group["normalize_grads"]:
                        norm_grad = norm_grad / (1e-30+torch.mean(norm_grad**2)**0.5)

                    p.add_(norm_grad.view(p.shape), alpha=-group["lr"])
